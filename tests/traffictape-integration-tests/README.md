...
`CaptureFromTestSuiteTest` boots a real Spring Boot app, drives HTTP through it, captures a corpus, then runs the CLI over that corpus and asserts the generated WireMock stubs and `test-plan.json` — including that an inbound case names the outbound stubs it depends on. It is the only test covering the whole loop, so treat a failure here as a product regression, not a flaky demo.
